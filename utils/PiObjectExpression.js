...
export default class PiObjectExpression extends PiExpression {
  ...
  format(indent = 0) {
    ...
    this._properties.forEach((prop, index) => {
      if (prop.spread) {
        const spreadStr = prop.spread.format(0).trim();
        formattedPropertyStrings.push(spreadStr);
        singleLineLength += spreadStr.length + (index < this._properties.length - 1 ? 2 : 0);
        return;
      }
      ...
      if (prop.keyToken.type !== TokenType.STR && !/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(keyStr)) {
        keyStr = `"${keyStr}"`;
      }
    ...
    if (useMultiLine) {
      ...
      this._properties.forEach((prop, index) => {
        ...
        if (prop.spread) {
          result += prop.spread.format(0);
          if (index < this._properties.length - 1) result += ",";
          result += "\n";
          return;
        }
  ...
  minify(context) {
    ...
    const parts = this._properties.map((prop) => {
      if (prop.spread) return prop.spread.minify(context);
      const key = prop.keyToken.value;
      const expr = prop.value;
      // If key needs quotes, add them for minified output
      let minifiedKey = prop.keyToken.type === TokenType.STR || /^[a-zA-Z_][a-zA-Z0-9_]*$/.test(key) ? key : `"${key}"`;
