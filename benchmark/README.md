...
The summary reports total runtime, total speed based on summed runtimes, and the geometric mean of the per-benchmark speedup ratios. The geometric mean gives each benchmark equal weight, so a long workload such as `binary_tree` does not dominate a short workload such as `call`.
...
Total speed:
  Pilang vs CPython: 1.45x faster
  Pilang vs Lua:     1.09x faster

Geometric mean:
  Pilang vs CPython: 1.80x faster
  Pilang vs Lua:     1.27x faster
```
...
* Total speed compares summed runtimes; the geometric mean summarizes individual benchmark speedups.
